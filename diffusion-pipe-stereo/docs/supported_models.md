...
transformer_dtype = 'float8'
stereo_timestep = 1.0
domain_label = 'converged'
```

`wan_stereo` is an experimental StereoPilot-style prototype. It trains a Wan T2V
backbone to map a left-eye video latent directly to a matching right-eye video
latent in one forward pass. Put right-eye target videos in the dataset directory
`path`, put left-eye videos with matching filenames in `control_path`, and set
`domain_label` to `parallel`/`0` or `converged`/`1`.

See [docs/wan_stereo.md](./wan_stereo.md) and the example configs
`examples/wan_stereo_1_3b.toml` and `examples/wan_stereo_dataset.toml`.
