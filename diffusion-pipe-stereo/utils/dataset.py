...
class DirectoryDataset:
    ...
    def __init__(self, directory_config, dataset_config, model_name, framerate=None, round_to_multiple=32, skip_dataset_validation=False):
        ...
        self.domain_label = self._normalize_domain_label(
            self.directory_config.get('domain_label', self.dataset_config.get('domain_label', None))
        )
    ...
    def _normalize_domain_label(self, value):
        if value is None:
            return None
        if isinstance(value, str):
            key = value.strip().lower()
            if key not in self.DOMAIN_LABELS:
                raise ValueError(
                    f"Unknown domain_label={value!r}. Expected one of "
                    f"{sorted(self.DOMAIN_LABELS)} or integer 0/1."
                )
            return self.DOMAIN_LABELS[key]
        value = int(value)
        if value not in (0, 1):
            raise ValueError(f'domain_label must be 0 or 1, got {value}')
        return value
    ...
    def _get_ungrouped_metadata(self, regenerate_cache=False, trust_cache=False):
        ...
        if regenerate_cache or not metadata_cache_file_1.exists() or not trust_cache:
            ...
            if self.domain_label is not None:
                d['domain_label'] = [self.domain_label] * len(image_specs)
    ...
    def _metadata_map_fn(self):
        ...
        def fn(example):
            ...
            if self.domain_label is not None:
                ret['domain_label'] = [example['domain_label'][0]]
            return ret
...
def _cache_fn(datasets, queue, preprocess_media_file_fn, num_text_encoders, regenerate_cache, trust_cache, caching_batch_size):
    ...
    def latents_map_fn(example, rank):
        ...
        has_domain_label = ('domain_label' in example)
        ...
        for i, (image_spec, mask_path, size_bucket, caption) in enumerate(
            ...
            if has_domain_label:
                domain_labels.extend([example['domain_label'][i]] * len(items))
            if is_edit_dataset:
        ...
        if len(tensors_and_masks) == 0:
            ...
            ret = {'latents': [], 'mask': [], 'image_spec': [], 'caption': []}
            if has_domain_label:
                ret['domain_label'] = []
            return ret
        ...
        if has_domain_label:
            results['domain_label'] = domain_labels
        return results
